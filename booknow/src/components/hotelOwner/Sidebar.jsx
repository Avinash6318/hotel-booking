...
const Sidebar = () => {
  ...
  return (
    <div className='md:w-64 w-16 border-r h-full text-base border-gray-300 pt-1
     flex flex-col tarnsition-all duration-300'>
        {sidebarLinks.map((item,index)=>(
            <NavLink to={item.path} key={index} end='/owner'
                className={({isActive})=>`flex items-center py-3 px-4 md:px-8 gap-3 
                ${isActive? "border-r-4 md:border-r-[6px] bg-blue-200 border-blue-600 text-blue-600":
                "hover:bg-gray-100/20 border-white text-gray-700"}`}>
